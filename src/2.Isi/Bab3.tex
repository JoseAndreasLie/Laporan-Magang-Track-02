...
Seluruh kegiatan magang dilaksanakan secara \textit{remote}, sehingga koordinasi tim dilakukan secara daring melalui \textit{platform Discord} dan \textit{repository management system} GitHub. Discord digunakan sebagai sarana komunikasi utama untuk berdiskusi, menyampaikan pembaruan progres, serta melakukan rapat mingguan. Sementara itu, GitHub digunakan untuk mengelola kode sumber, melakukan \textit{pull request}, serta memfasilitasi proses \textit{code review} dan kolaborasi antar anggota tim.

Setiap minggu diadakan \textit{sprint meeting} yang dipimpin oleh Raditya untuk membahas perkembangan proyek, hambatan teknis, serta rencana pengembangan fitur berikutnya. Selain itu, setiap anggota tim juga melakukan \textit{asynchronous daily updates} melalui Discord yang berisi laporan progres harian dan kendala yang dihadapi. Evaluasi hasil kerja dilakukan secara berkala melalui sesi \textit{code review} dan \textit{sprint retrospective} untuk memastikan kualitas pengembangan tetap terjaga serta selaras dengan standar teknis perusahaan.
...
\begin{enumerate}
    \item Mengembangkan dan memelihara API untuk aplikasi \textit{Internal System}, termasuk pembuatan fitur baru, penyempurnaan fungsi yang sudah ada, serta memastikan kompatibilitas dengan sistem lain yang akan menggunakan produk ini.
    ...
    \item Melakukan koordinasi dan kolaborasi dengan tim pengembang melalui Discord dan GitHub terkait pembagian tugas, pembahasan revisi, serta proses integrasi antar modul.
\end{enumerate}
...
\begin{center}
    \begin{longtable}{|c|p{0.75\textwidth}|}
        ...
        \hline
        4 & Revamp pada modul \textit{User Management}, \textit{Leave Management}, dan \textit{Permit Detail}, termasuk penyempurnaan validator autentikasi serta fungsi perhitungan hari untuk meningkatkan konsistensi sistem. \\
        \hline
        5 & Melakukan penyempurnaan sistem dengan menambah API baru untuk persetujuan cuti, memperbaiki fungsi perhitungan hari, melakukan refaktor pada fungsi pengguna dan paginasi, serta memperbaiki data terkait proyek, produk, dan jabatan. \\
        \hline
        ...
        \hline
        8 & Melakukan pengembangan fitur ekspor data ke format XLSX, perbaikan API pembuatan pengguna, penyempurnaan berkas migrasi dan seeding, serta penambahan sistem perizinan baru pada setiap modul. \\
        \hline
        ...
        \hline
        10 & Melanjutkan pengembangan fitur onboarding klien dengan penyempurnaan sistem pembacaan berkas CSV, pembuatan tabel serta fungsi Import Mapping, dan perbaikan FileController untuk mendukung proses impor data secara efisien. \\
        \hline
...
% Setelah data selesai disimpan, \textit{Superadmin} dapat mengakses modul \textit{Salary Slip} untuk melakukan finalisasi gaji. Finalisasi ini memungkinkan pengecekan akhir terhadap rincian gaji sebelum tanggal gajian. Di PT Visi Karya Nusantara, proses penggajian dilakukan setiap tanggal 25, sehingga proses finalisasi disarankan dilakukan pada tanggal 24 setiap bulannya. Setelah tanggal 25, data tidak dapat lagi diubah.
...
% % -------------------- %
% Setelah data gaji pegawai selesai dibuat, \textit{Superadmin} dapat mengakses modul \textit{Salary Slip} untuk melakukan finalisasi gaji. Proses finalisasi ini memungkinkan pengecekan akhir terhadap rincian gaji sebelum tanggal gajian. Di PT Visi Karya Nusantara, proses penggajian dilakukan setiap tanggal 25, sehingga proses finalisasi disarankan dilakukan pada tanggal 24 setiap bulannya. Setelah tanggal 25, data tidak dapat lagi diubah.
% \begin{figure}[H]
%     \centering
